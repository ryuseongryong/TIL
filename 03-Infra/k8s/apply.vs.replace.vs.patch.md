...
때때로 쿠버네티스 API에서 허용하지 않는 리소스를 변경해야 하는 경우가 있습니다. 이러한 경우 리소스를 강제로 대체하여 리소스를 효과적으로 삭제하고 다시 생성할 수 있습니다. 이 작업은 명령줄에서 kubectl replace --force를 사용하여 수행한다. 이렇게 하면 리소스를 강제로 즉시 삭제한 다음 제공된 사양으로 다시 생성한다. "강제 대체" API 엔드포인트는 없다. Kubernetes API를 사용하여 유사한 작업을 수행하려면 두 가지 작업을 별도로 수행해야 합니다. 먼저 리소스를 삭제하고 gracePeriodSeconds를 0(0)으로 설정하고 propagationPolicy를 "Background"로 설정한 다음 원하는 스펙을 사용하여 리소스를 다시 생성해야 합니다.

이러한 접근 방식은 혼란을 야기하고 일관성 없는 상태를 초래할 수 있습니다!

## Server-side apply
위에서 언급했듯이, 쿠버네티스 개발자들은 쿠버네티스 API 내의 kubectl에서 적용 로직을 구현하기 위해 노력하고 있다. 베타 2 서버 측 적용 로직은 쿠버네티스 1.18에서 명령줄에서 kubectl apply --server-side를 통해 사용할 수 있으며, 리소스 API 엔드포인트에서 콘텐츠 유형 헤더 값이 application/apply-patch+yaml로 설정된 PATCH 요청 메서드를 사용하여 사용할 수 있다.

참고: JSON은 유효한 YAML이므로 콘텐츠 유형이 애플리케이션/애플리케이션-패치+앰엘일 때 JSON 사양을 전송할 수 있습니다.

단순히 API를 통해 모든 클라이언트가 kubectl 적용 로직을 사용할 수 있도록 하는 것 외에도, 서버 측 적용 구현은 누가 스펙의 어떤 필드를 담당하는지 추적하여 여러 "필드 관리자"가 무의식적으로 충돌하지 않고 안전하게 스펙의 다른 부분을 편집할 수 있도록 한다. 다시 말해, 서버 측 적용이 더 일반적으로 사용 가능해지면 서로 다른 클라이언트(예: kubectl, Pulumi 또는 Terraform, GitOps 및 Kubernetes 클라이언트 라이브러리를 사용하여 작성한 스크립트)가 모두 동일한 리소스를 안전하게 관리할 수 있는 메커니즘이 제공됩니다.

## Wrapping up(dates)

쿠버네티스 클러스터에서 리소스를 업데이트하는 다양한 방법을 간략하게 살펴본 이번 여행이 도움이 되었기를 바랍니다. 우리는 단순히 적용 대 교체가 아니라 적용, 편집, 패치 또는 교체로 리소스를 업데이트할 수 있다는 것을 배웠습니다. 그리고 실제로는 쿠버네티스 적용 대 교체 또는 쿠버네티스 적용 대 패치가 아니라 각 접근 방식에는 특정 사용 사례를 염두에 두고 있습니다. 원자 업데이트가 필요한 경우, kubectl replace 또는 PUT/replace/update API 호출을 사용하는 것이 좋다. 그렇지 않은 경우, 일반적으로 kubectl apply 또는 PATCH API 호출을 통해 전략적 병합 패치를 사용하는 것이 최선의 선택이다. 최소한 'kubernetes 적용 대 교체'를 검색할 때 구글이나 스택 오버플로를 신뢰해서는 안 된다는 것을 배웠기를 바란다. 즉, 이 게시물이 현재 답변을 대체할 때까지는 그렇습니다.

Kubernetes 리소스를 업데이트하는 방식에 관계없이, Kubernetes 포드 상태 모니터 스킬로 클러스터를 모니터링하여 개발자와 운영자의 부담을 덜어주세요.
