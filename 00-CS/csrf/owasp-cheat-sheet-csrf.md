...
싱크로나이저 토큰과 이중 제출 쿠키는 양식 데이터의 위조를 방지하는 데 사용되지만 구현이 까다롭고 사용성을 저하시킬 수 있습니다. 많은 최신 웹 애플리케이션은 <form> 태그를 사용하지 않습니다. 특히 AJAX 또는 API 엔드포인트에 적합한 사용자 친화적인 방어 방법은 사용자 지정 요청 헤더를 사용하는 것입니다. 이 접근 방식에는 토큰이 필요하지 않습니다.

이 패턴에서는 클라이언트가 CSRF 보호가 필요한 요청에 사용자 지정 헤더를 추가합니다. 헤더는 기존 헤더와 충돌하지 않는 한 임의의 키-값 쌍이 될 수 있습니다.
...
요청을 처리할 때 API는 이 헤더의 존재 여부를 확인합니다. 헤더가 존재하지 않으면 백엔드에서 위조 가능성이 있는 요청으로 거부합니다. 이 접근 방식에는 몇 가지 장점이 있습니다:

    - UI 변경이 필요하지 않습니다.
    - 토큰을 추적하기 위해 서버 상태가 도입되지 않습니다.

클라이언트의 어느 곳에서나 <form> 태그를 사용하는 경우에도 토큰 등 이 문서에 설명된 다른 접근 방식을 사용하여 보호해야 합니다.

이 방어 방식은 브라우저의 동일 출처 정책(SOP) 제한에 따라 자바스크립트만 해당 출처 내에서만 사용자 정의 헤더를 추가하는 데 사용할 수 있습니다. 기본적으로 브라우저는 자바스크립트가 사용자 지정 헤더를 사용하여 교차 오리진 요청을 하는 것을 허용하지 않습니다. 오리진에서 제공하는 자바스크립트만 이러한 헤더를 추가할 수 있습니다.
