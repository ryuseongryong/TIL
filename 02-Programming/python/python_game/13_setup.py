...
def setup_gemstone():
    ...
    gemstone_group.add(
        Gemstone(gemstone_images[0], (400, 150), small_gold_price, small_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[0], (600, 700), small_gold_price, small_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[0], (1200, 210), small_gold_price, small_gold_speed))

    # 큰 금
    gemstone_group.add(
        Gemstone(gemstone_images[1], (300, 500), big_gold_price, big_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[1], (180, 600), big_gold_price, big_gold_speed))

    # 돌
    gemstone_group.add(
        Gemstone(gemstone_images[2], (300, 380), stone_price, stone_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[2], (700, 330), stone_price, stone_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[2], (1000, 480), stone_price, stone_speed))
    # 다이아몬드
    gemstone_group.add(
        Gemstone(gemstone_images[3], (900, 420), diamond_price, diamond_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[3], (1200, 700), diamond_price, diamond_speed))


def update_score(score):
    # 전역 변수를 수정하기 위해서 가져오기
    global curr_score
    curr_score += score
...
def display_score():
    txt_curr_score = game_font.render(
        f"Curr Score : {curr_score:,}", True, WHITE)
    screen.blit(txt_curr_score, (50, 20))

    txt_goal_score = game_font.render(
        f"Goal Score : {goal_score:,}", True, WHITE)
    screen.blit(txt_goal_score, (50, 80))


def display_time(time):
    txt_timer = game_font.render(f"Time : {time}", True, WHITE)
...
#! 게임 종료 관련 변수
game_result = None  # 게임 결과 complete or fail
total_time = 60  # 총 시간
...
#! 배경 이미지 불러오기
# 현재 위치 반환(pyinstaller에서 오류가 발생하여 수정)
current_path = os.getenv('MINE_GAME')
