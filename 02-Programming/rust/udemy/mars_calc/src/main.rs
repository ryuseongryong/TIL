...
fn main() {
    ...
    // ```
        ...
        // 즉 참조는 단지 포인터일 뿐이라는 것이다.

    // 함수 own_string에서
    // 이 함수에 다시 전체 구조를 갖고 있다.
    // 왜냐하면 문자열의 소유권이 이 함수로 이동했고, 전체 문자열이 물리적으로 이동했기 때문이다.
    // 그래서 힙에 있는 값은 여전히 동일하고, 이 값은 건드리지 않았다.
    // 하지만 메타데이터와 함께 이 전체 데이터 구조는 물리적으로 메모리에 있는 
    // 이 함수의 스택 프레임으로 이동했다.
    // 그래서 이제 함수는 문자열 구조의 포인터를 갖는 것이 아니라 문자열 구조를 소유한다.
    // 그리고 이 값이 범위를 벗어났을 때, 힙에 저장하고 있는 포인터를 해제한다.
    // 이것은 중요한 차이점이다.
    // 참조를 전달한다는 것은 이미 갖고 있는 데이터에 대한 포인터를 생성한다는 의미이고,
