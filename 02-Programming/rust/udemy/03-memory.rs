...
// #0 stack_and_heap::stack_and_heap () at stack-and-heap/src/main.rs:13
// #1 0x0000555555558899 in stack_and_heap::stack_only (b=2) at stack-and-heap/src/main.rs:9
// #2 0x000055555555869b in stack_and_heap::main () at stack-and-heap/src/main.rs:3
// (More stack frames follow...)

// 스택 프레임 맨 위에는 stack_and_heap 함수가 있다.
// 그 아래에는 stack_only 함수
// 그 아래에는 main 함수가 있다.
// 인수나 지역변수는 아직 없다. 새로운 스택 프레임이고 저장된 것이 아무것도 없는 상태이다.
// 다음 줄로 진행하면, 지역변수 정수 d가 저장되어 있다.
// 그 다음 줄로 진행하면, 힙에 메모리를 할당한다.
// 그리고 지역 변수를 체크해보면, e에 0x555555787a40이 할당되어 있다. 힙에 할당한 메모리의 주소이고 e의 값 7이 있는 위치이다.
// 그래서 이 주소에서 저장된 값을 읽을 수 있다. 숫자 7이 있다는 것을 알 수 있다. 이 주소를 참조하면 거기 저장된 값을 얻을 수 있다.
// stack_and_heap 함수를 종료하고, 이 주소를 다시 읽어보면 값이 0인 것을 확인할 수 있다.
// 스마트 포인터(Box)가 함수 종료 후 범위를 벗어나게 되면 Box는 가리키던 해당 메모리를 확실하게 해제한다.
// 스택 프레임을 체크해보면, 맨위에는 stack_only 함수가 있다. stack_and_heap 함수의 메모리는 이제 해제되었다.
// 계속 진행하면 main 함수로 돌아가고 스택 프레밈 가장 위에는 main 함수가 위치한다.
// 그리고 그 아래에는 실제로 프로세스를 시작하는 랭귀지 관련 함수들이 있다.(신경쓸 필요는 없음)
// 중요한 것은 stack_and_heap과 stack_only 함수에 대한 스택 프레임은 해제되었다는 것이다.
// 함수를 계속 실행하면 이런 모든 숫자들을 출력하면서 프로그램은 종료된다.
// 이것이 프로그램을 실행할 때 벌어지는 일들이다.
